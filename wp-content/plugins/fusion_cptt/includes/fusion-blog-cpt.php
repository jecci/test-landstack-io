...
if ( fusion_is_element_enabled( 'fusion_blog_cpt' ) ) {
	...
	if ( ! class_exists( 'FusionSC_Blog_CPT' ) ) {
		...
		class FusionSC_Blog_CPT extends Fusion_Element {
			...
			public function __construct() {
				parent::__construct();
				// Containers.
				add_action( 'fusion_blog_cpt_shortcode_before_loop', [ $this, 'before_loop' ] );
				add_action( 'fusion_blog_cpt_shortcode_before_loop_timeline', [ $this, 'before_loop_timeline' ] );
				add_action( 'fusion_blog_cpt_shortcode_after_loop', [ $this, 'after_loop' ] );

				// Post / loop basic structure.
				add_action( 'fusion_blog_cpt_shortcode_loop_header', [ $this, 'loop_header' ] );
				add_action( 'fusion_blog_cpt_shortcode_loop_footer', [ $this, 'loop_footer' ] );
				add_action( 'fusion_blog_cpt_shortcode_loop_content', [ $this, 'loop_content' ] );
				add_action( 'fusion_blog_cpt_shortcode_loop_content', [ $this, 'page_links' ] );
				add_action( 'fusion_blog_cpt_shortcode_loop', [ $this, 'loop' ] );

				// Special blog layout structure.
				add_action( 'fusion_blog_cpt_shortcode_wrap_loop_open', [ $this, 'wrap_loop_open' ] );
				add_action( 'fusion_blog_cpt_shortcode_wrap_loop_close', [ $this, 'wrap_loop_close' ] );
				add_action( 'fusion_blog_cpt_shortcode_date_and_format', [ $this, 'add_date_box' ] );
				add_action( 'fusion_blog_cpt_shortcode_date_and_format', [ $this, 'add_format_box' ] );
				add_action( 'fusion_blog_cpt_shortcode_timeline_date', [ $this, 'timeline_date' ] );

				// Element attributes.
				add_filter( 'fusion_attr_blog_cpt-shortcode', [ $this, 'attr' ] );
				add_filter( 'fusion_attr_blog_cpt-shortcode-posts-container', [ $this, 'posts_container_attr' ] );
				add_filter( 'fusion_attr_blog_cpt-shortcode-loop', [ $this, 'loop_attr' ] );
				add_filter( 'fusion_attr_blog_cpt-shortcode-post-title', [ $this, 'post_title_attr' ] );
				add_filter( 'fusion_attr_blog_cpt-shortcode-post-content-wrapper', [ $this, 'post_content_wrapper_attr' ] );
				add_filter( 'fusion_attr_blog_cpt-fusion-post-wrapper', [ $this, 'post_wrapper_attr' ] );
				add_filter( 'fusion_attr_blog_cpt-fusion-content-sep', [ $this, 'content_sep_attr' ] );

				add_shortcode( 'fusion_blog_cpt', [ $this, 'render' ] );

				// Ajax mechanism for query related part.
				add_action( 'wp_ajax_get_fusion_blog_cpt', [ $this, 'ajax_query' ] );
			}
			...
			public function render( $args, $content = '' ) {
				...
				$defaults = FusionBuilder::set_shortcode_defaults( $this->defaults, $args, 'fusion_blog_cpt' );
			...
			public function loop_header() {
				...
				if ( '' !== $link ) {
					...
					$link      = '<' . $title_tag . ' ' . FusionBuilder::attributes( 'blog_cpt-shortcode-post-title' ) . '>' . $link . '</' . $title_tag . '>';
				}
			...
			public function loop_footer() {
				...
				if ( 0 < $this->args['meta_info_combined'] && in_array( $this->args['layout'], [ 'large', 'medium' ], true ) ) {
					echo '<div class="fusion-meta-info">' . cptt_avada_render_post_metadata( 'standard', $this->meta_info_settings ) . $this->read_more() . '</div>'; // phpcs:ignore WordPress.Security.EscapeOutput
				}
...
add_filter( 'redirect_canonical', 'fusion_blog_redirect_canonical_cpt' );
/**
 ...
 * @return string
 */
function fusion_blog_redirect_canonical_cpt( $redirect_url ) {
...
function fusion_element_blog_cpt() {
    ...
    $am_custom_taxonomies = Fusion_Cptt_AvadaCPTTHelper::am_custom_taxonomy_array();
