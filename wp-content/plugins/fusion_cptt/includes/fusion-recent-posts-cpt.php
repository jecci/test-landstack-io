...
if (!class_exists('FusionSC_RecentPosts_CPT') && class_exists('FusionCore_Plugin')) {
    ...
    class FusionSC_RecentPosts_CPT extends Fusion_Element  {
            ...
            public function __construct() {
                parent::__construct();
                add_filter( 'avada_force_enqueue', '__return_true' );
                add_filter( 'fusion_attr_recentposts_cpt-shortcode', [ $this, 'attr' ] );
                add_filter( 'fusion_attr_recentposts_cpt-shortcode-section', [ $this, 'section_attr' ] );
                add_filter( 'fusion_attr_recentposts_cpt-shortcode-column', [ $this, 'column_attr' ] );
                add_filter( 'fusion_attr_recentposts_cpt-shortcode-content', [ $this, 'content_attr' ] );
                add_filter( 'fusion_attr_recentposts_cpt-shortcode-slideshow', [ $this, 'slideshow_attr' ] );
                add_filter( 'fusion_attr_recentposts_cpt-shortcode-img', [ $this, 'img_attr' ] );
                add_filter( 'fusion_attr_recentposts_cpt-shortcode-img-link', [ $this, 'link_attr' ] );

                add_shortcode( 'fusion_recent_posts_cpt', [ $this, 'render' ] );

                // Ajax mechanism for query related part.
                add_action( 'wp_ajax_get_fusion_recent_posts_cpt', [ $this, 'ajax_query' ] );
            }
            ...
            public function query( $defaults ) {
                ...
                if ( isset( $_POST['model'] ) ) { // phpcs:ignore WordPress.Security.NonceVerification
                    ...
                    $defaults     = FusionBuilder::set_shortcode_defaults( self::get_element_defaults(), $defaults, 'fusion_recent_posts_cpt' );
                    $live_request = true;
                }
                    ...
                    if ( 'yes' === $thumbnail && 'date-on-side' !== $layout && ! post_password_required( get_the_ID() ) ) {
                        ...
                        if ( has_post_thumbnail() || $post_video ) {
                            ...
                            if ( has_post_thumbnail() ) {
                                ...
                                $slides .= '<li><a href="' . get_permalink( get_the_ID() ) . '" ' . FusionBuilder::attributes( 'recentposts_cpt-shortcode-img-link' ) . '>' . $attachment_image . '</a></li>';
                            }
