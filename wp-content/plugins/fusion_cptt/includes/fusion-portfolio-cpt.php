...
if ( function_exists( 'fusion_is_element_enabled' ) && fusion_is_element_enabled( 'fusion_portfolio_cpt' ) ) {

	if ( ! class_exists( 'FusionSC_Portfolio_CPT' ) && class_exists( 'Fusion_Element' ) ) {
		/**
		 * Shortcode class.
		 *
		 * @package fusion-core
		...
		class FusionSC_Portfolio_CPT extends Fusion_Element {
			...
			public function __construct() {
				parent::__construct();
				add_action( 'fusion_portfolio_shortcode_content_cpt', [ $this, 'get_post_content' ] );

				// Element attributes.
				add_filter( 'fusion_attr_portfolio-shortcode_cpt', [ $this, 'attr' ] );
				add_filter( 'fusion_attr_portfolio-shortcode_cpt-portfolio-wrapper', [ $this, 'portfolio_wrapper_attr' ] );
				add_filter( 'fusion_attr_portfolio-shortcode_cpt-portfolio-content', [ $this, 'portfolio_content_attr' ] );
				add_filter( 'fusion_attr_portfolio-shortcode_cpt-carousel', [ $this, 'carousel_attr' ] );
				add_filter( 'fusion_attr_portfolio-shortcode_cpt-slideshow', [ $this, 'slideshow_attr' ] );
				add_filter( 'fusion_attr_portfolio-shortcode_cpt-filter-link', [ $this, 'filter_link_attr' ] );
				add_filter( 'fusion_attr_portfolio-fusion-portfolio_cpt-content-wrapper', [ $this, 'portfolio_content_wrapper_attr' ] );
				add_filter( 'fusion_attr_portfolio-fusion-content_cpt-sep', [ $this, 'content_sep_attr' ] );

				add_shortcode( 'fusion_portfolio_cpt', [ $this, 'render' ] );
			...
			public function query() {
                ...
                //Amunet
                $taxonomy_raw = self::$static_args['cus_taxonomy'];
                preg_match("/(.+)__(.+)/", $taxonomy_raw, $taxonomy_match_array);
                $taxonomy_name_real = $taxonomy_match_array[2];

                //get terms - cat_slug
                if (!empty(self::$static_args['cus_terms'])) {
                    $term_name_real = array();
                    $taxonomy_terms_raw = explode(',', self::$static_args['cus_terms']);
                ...
                if (!empty(self::$static_args['cus_terms_exclude'])) {
                    $term_name_exclude_real = array();
                    $taxonomy_terms_exclude_raw = explode(',', self::$static_args['cus_terms_exclude']);
                ...
                self::$static_args['taxonomy_name_real'] =$taxonomy_name_real;

                //Amunet End

				// Transform $cat_slugs to array.
				$cat_slugs = [];
					if ( self::$static_args['cus_terms'] ) {
				...
				$cats_to_exclude = [];
					...
					if ( self::$static_args['cus_terms_exclude'] ) {
				...
				$static_args = [
					'post_type'      => self::$static_args['cpt_post_type'],
					'paged'          => $paged,
                ...
                $static_args['meta_query'] = array(
                    'relation' => 'OR',
                    array(
                        'key'     => self::$static_args['sorting_key'],
                        'compare' => 'EXISTS',
                    ),
                    array(
                        'key'     => self::$static_args['sorting_key'],
				...
				$static_args['posts_per_page'] = '0' === $static_args['posts_per_page'] ? get_option( 'posts_per_page' ) : $static_args['posts_per_page'];

				if ( self::$static_args['offset'] ) {
					$static_args['offset']       = self::$static_args['offset'];
					self::$static_args['offset'] = self::$static_args['offset'] + ( $paged - 1 ) * $static_args['posts_per_page'];
				}
				...
				if ( ! empty( $cats_to_exclude) ) {
					...
					$static_args['tax_query'] = [ // phpcs:ignore WordPress.DB.SlowDBQuery
						[
					...
					if ( ! empty( $cat_slugs) ) {
						$static_args['tax_query']['relation'] = 'AND';
						$static_args['tax_query'][]           = [
							'taxonomy' => $taxonomy_name_real,
							'field'    => 'slug',
							'terms'    => $cat_slugs,
							'operator' => 'IN',
				...
				if ( $live_request ) {
					$static_args['post_status'] = 'publish';
				}

				$static_args['portfolio_sc_query'] = true;

				$portfolio_query = FusionCore_Plugin::fusion_core_cached_query( apply_filters( 'fusion_portfolio_query_args', $static_args ) );
				...
				if ( $portfolio_query->have_posts() ) {
					while ( $portfolio_query->have_posts() ) {
						...
						$post_categories = get_the_terms( $id, $taxonomy_name_real );

						$project_url = fusion_get_page_option( 'project_url', $id );

						$term_list = get_the_term_list( $id, $taxonomy_name_real, '<div class="fusion-carousel-meta">', ', ', '</div>' );
						...
						$the_cats = get_the_term_list( $id, $taxonomy_name_real, '', ', ', '' );
			 ...
			 * @access public
			 * @since 1.0
			 * @param  array  $static_args    Shortcode parameters.
			 * @param  string $content Content between shortcode.
			 * @return string          HTML output.
			 */
			public function render( $static_args, $content = '' ) {

                $taxonomy_raw = $static_args['cus_taxonomy'];
                preg_match("/(.+)__(.+)/", $taxonomy_raw, $taxonomy_match_array);
                $taxonomy_name_real = $taxonomy_match_array[2];
				...
				$defaults = FusionBuilder::set_shortcode_defaults( self::get_element_defaults(), $static_args, 'fusion_portfolio' );

				$defaults['column_spacing'] = FusionBuilder::validate_shortcode_attr_value( $defaults['column_spacing'], '' );

				if ( ! isset( $static_args['portfolio_layout_padding'] ) ) {
					$padding_values           = [];
					$padding_values['top']    = ( isset( $static_args['padding_top'] ) && '' !== $static_args['padding_top'] ) ? $static_args['padding_top'] : Fusion_Sanitize::size( $fusion_settings->get( 'portfolio_layout_padding', 'top' ) );
					$padding_values['right']  = ( isset( $static_args['padding_right'] ) && '' !== $static_args['padding_right'] ) ? $static_args['padding_right'] : Fusion_Sanitize::size( $fusion_settings->get( 'portfolio_layout_padding', 'right' ) );
					$padding_values['bottom'] = ( isset( $static_args['padding_bottom'] ) && '' !== $static_args['padding_bottom'] ) ? $static_args['padding_bottom'] : Fusion_Sanitize::size( $fusion_settings->get( 'portfolio_layout_padding', 'bottom' ) );
					$padding_values['left']   = ( isset( $static_args['padding_left'] ) && '' !== $static_args['padding_left'] ) ? $static_args['padding_left'] : Fusion_Sanitize::size( $fusion_settings->get( 'portfolio_layout_padding', 'left' ) );
				...
				if ( 'grid' === $defaults['layout'] && ! isset( $static_args['text_layout'] ) ) {
					...
					if ( has_post_thumbnail() || $fusion_settings->get( 'featured_image_placeholder' ) || $video_meta ) {
						...
						if ( 'carousel' === $layout ) {
							...
							if ( 'title_on_rollover' === $carousel_layout ) {
								...
								// Get the terms.
								$carousel_terms = get_the_term_list( $this->post_id, $taxonomy_name_real, '<div class="fusion-carousel-meta">', ', ', '</div>' );
							...
							$post_categories = get_the_terms( $this->post_id, $taxonomy_name_real );
							...
							if ( 'carousel' !== self::$static_args['layout'] && 'no_text' !== self::$static_args['text_layout'] ) {
								...
								// Get the post terms.
								if ( $categories ) {
									$the_cats = get_the_term_list( $this->post_id, $taxonomy_name_real, '', ', ', '' );
									if ( $the_cats ) {
										$post_terms = '<div class="fusion-portfolio-meta">' . $the_cats . '</div>';
									}
								}
								 ...
								 */
								do_action( 'fusion_portfolio_shortcode_content_cpt' );
								...
								if ( 'boxed' === $text_layout && $stripped_content && 'masonry' !== self::$static_args['layout'] ) {
									$separator = '<div ' . FusionBuilder::attributes( 'portfolio-fusion-content_cpt-sep' ) . '></div>';
								}
								...
								// Put it all together.
								$post_content  = '<div ' . FusionBuilder::attributes( 'portfolio-shortcode_cpt-portfolio-content' ) . '>';
				...
				if ( 'carousel' === $layout ) {
					...
					$html = '<div ' . FusionBuilder::attributes( 'portfolio-shortcode_cpt' ) . '><div ' . FusionBuilder::attributes( 'portfolio-shortcode_cpt-carousel' ) . '>' . $main_carousel . $navigation . '</div></div>';
					...
					if ( $portfolio_categories ) {
						...
						if ( 'yes-without-all' !== $filters ) {
							$filter       = '<li role="presentation" ' . FusionBuilder::attributes( 'fusion-filter fusion-filter-all fusion-active' ) . '><a ' . FusionBuilder::attributes(
								'portfolio-shortcode_cpt-filter-link',
								[
									'data-filter' => '*',
								]
					...
					// Put it all together.
					$html = $styling . '<div ' . FusionBuilder::attributes( 'portfolio-shortcode_cpt' ) . '>' . $filter_wrapper . $styles . '<div ' . FusionBuilder::attributes( 'portfolio-shortcode_cpt-portfolio-wrapper' ) . '>' . $portfolio_posts . '</div>' . $pagination . '</div>';
				...
				return apply_filters( 'fusion_element_portfolio_content', $html, $static_args );
			 ...
			 * @access public
			 * @since 1.0
			 * @param array $static_args The arguments array.
			 * @return array
			 */
			public function portfolio_wrapper_attr( $static_args ) {
			 ...
			 * @return array
			 */
			public function portfolio_content_attr( $static_args ) {
				global $fusion_library;
			 ...
			 * @return array
			 */
			public function filter_link_attr( $static_args ) {

				$attr = [
					'href' => '#',
				];

				if ( $static_args['data-filter'] ) {
					$attr['data-filter'] = $static_args['data-filter'];
				}
...
function fusion_element_portfolio_cpt() {
    ...
    $am_post_types = Fusion_Cptt_AvadaCPTTHelper::am_custom_post_type_array();
    $am_all_taxonomies = Fusion_Cptt_AvadaCPTTHelper::am_all_taxonomy_array();
    $am_custom_terms = Fusion_Cptt_AvadaCPTTHelper::am_custom_terms_array();
    $am_custom_fields = Fusion_Cptt_AvadaCPTTHelper::am_get_meta_key();
