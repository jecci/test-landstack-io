		...
		foreach ( $social_networks as $key => $value ) {
			...
			foreach ( $value as $network => $link ) {
				...
				if ( 'custom' === $network && is_array( $link ) ) {
					...
					if ( is_array( $social_media_icons ) ) {
						$search_name = 'mail' === $network ? 'email' : $network;
						$key = array_search( $search_name, $social_media_icons['icon'], true );
...
function fusion_element_options_loop( $params, $class = '' ) {
	...
	function fusion_display_option( param ) {
		...
		<?php do_action( 'fb_ai_params' ); // phpcs:ignore WordPress.Security.EscapeOutput ?>
				...
				<div class="option-details">
					<# if ( 'undefined' !== typeof param.heading ) { #>
						<h3><span>{{ heading }}</span>
							...
							<?php do_action( 'fb_ai_links' ); // phpcs:ignore WordPress.Security.EscapeOutput ?>
						</h3>
					<# }; #>
...
if ( ! function_exists( 'awb_get_user_meta_data_fields' ) ) {
	/**
	 * Gets user meta data fields.
	 *
	 * @access public
	 * @since 3.14.0
	 * @return array
	 */
	function awb_get_user_meta_data_fields() {
		$meta_data = [
			'user_login'       => __( 'Username', 'fusion-builder' ),
			'first_name'       => __( 'First Name', 'fusion-builder' ),
			'last_name'        => __( 'Last Name', 'fusion-builder' ),
			'nickname'         => __( 'Nickname', 'fusion-builder' ),
			'user_email'       => __( 'Email', 'fusion-builder' ),
			'user_url'         => __( 'Website', 'fusion-builder' ),
			'description'      => __( 'Biographical Info', 'fusion-builder' ),
			'ID'               => __( 'User ID', 'fusion-builder' ),
			'user_registered'  => __( 'Registration Date', 'fusion-builder' ),
			'author_email'     => __( 'Email ( Author Page )', 'fusion-builder' ),
			'author_facebook'  => __( 'Facebook (Author Page)', 'fusion-builder' ),
			'author_twitter'   => __( 'X (Author Page)', 'fusion-builder' ),
			'author_linkedin'  => __( 'LinkedIn (Author Page)', 'fusion-builder' ),
			'author_dribble'   => __( 'Dribble (Author Page)', 'fusion-builder' ),
			'author_whatsapp'  => __( 'WhatsApp (Author Page)', 'fusion-builder' ),
			'author_custom'    => __( 'Custom Message (Author Page)', 'fusion-builder' ),
		];
		...
		if ( class_exists( 'WooCommerce' ) ) {
			...
			if ( $user_id ) {
				...
				$woo_meta  = array_filter(
					$user_meta,
					function( $key ) {
						return str_starts_with( $key, 'billing_' ) || str_starts_with( $key, 'shipping_' );
					},
					ARRAY_FILTER_USE_KEY
				);

				foreach( $woo_meta as $meta_name => $value ) {
					$meta_data[ $meta_name ] = ucwords( str_replace( '_', ' ', $meta_name ) );
				}
			}
		}
...
function awb_get_user_meta_value( $args ) {
	if ( ! empty( $args['user_id'] ) ) {
		if ( is_numeric( $args['user_id'] ) ) {
			$user = get_user_by( 'ID', $args['user_id'] );
		} elseif ( false !== strpos( $args['user_id'], '@' ) ) {
			$user = get_user_by( 'email', $args['user_id'] );
		} else {
			$user = get_user_by( 'login', $args['user_id'] );
		}
	} else {
	...
	$user_meta_key  = ! empty( $args['meta_key_custom'] ) ? $args['meta_key_custom'] : $args['meta_key'];
	$user_meta_key  = $user_meta_key ? $user_meta_key : 'nicknanme';
	$meta_value     = '';
	
	// Some data is only available in user data object.
	$user_data_keys = [ 'ID', 'user_login', 'user_email', 'user_url' ];
	$user_object    = in_array( $user_meta_key, $user_data_keys ) ? 'user_data' : 'user_meta';

	if ( $user_id ) {
		if ( 'user_data' === $user_object ) {
			$user_data  = get_userdata( $user_id );
			$meta_value = isset( $user_data->data ) && isset( $user_data->data->$user_meta_key ) ? $user_data->data->$user_meta_key : '';
		} else {
			$meta_value = get_user_meta( $user_id, $user_meta_key, true );
