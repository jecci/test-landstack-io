		...
		switch ( $type ) {
			...
			default:
				if ( '' !== $setting_value ) {
					...
					$setting_link = str_replace( [ '</a>', '</span>' ], [ apply_filters( 'fusion_options_label', esc_html__( 'Element Options', 'fusion-builder' ) ) . '</a>', apply_filters( 'fusion_options_label', esc_html__( 'Element Options', 'fusion-builder' ) ) . '</span>' ], $setting_link );

					/* translators: %1$s is the link. %2$s is the link text. */
					$setting_description = sprintf( __( '  Currently no default selected. Can be set globally from the %s.', 'fusion-builder' ), $setting_link );
				}
				break;
		}
		$default_value = '';
