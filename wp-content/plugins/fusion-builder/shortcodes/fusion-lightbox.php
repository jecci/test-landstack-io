	...
	if ( ! class_exists( 'FusionSC_FusionLightbox' ) ) {
		...
		class FusionSC_FusionLightbox extends Fusion_Element {
			...
			public function attr() {
				...
				if ( '' === $this->args['type'] ) {
					if ( $this->args['full_image_id'] ) {
						$image_data = fusion_library()->images->get_attachment_data_by_helper( $this->args['full_image_id'], $this->args['full_image'] );
						$this->args['full_image'] = isset( $image_data['url'] ) ? $image_data['url'] : $this->args['full_image'];
					}
			...
			public function image_attr() {
				...
				if ( $this->args['thumbnail_image'] ) {
					...
					if ( $this->args['thumbnail_image_id'] ) {
						$image_data = fusion_library()->images->get_attachment_data_by_helper( $this->args['thumbnail_image_id'], $this->args['thumbnail_image'] );

						if ( ! empty( $image_data['url'] ) ) {
							$this->args['thumbnail_image'] = isset( $image_data['url'] ) ? $image_data['url'] : $this->args['thumbnail_image'];
				...
				$attr['alt'] = $this->args['alt_text'] ? $this->args['alt_text'] : $image_alt;
	...
	fusion_builder_map(
		[
			...
			'params'    => [
				...
				[
					'type'        => 'textfield',
					'heading'     => esc_attr__( 'Thumbnail Image ID', 'fusion-builder' ),
					'description'  => esc_attr__( 'Clicking this image will show lightbox.', 'fusion-builder' ),
					'param_name'  => 'thumbnail_image_id',
					'value'       => '',
					'hidden'      => true,
				],					
				[
				...
				[
					'type'        => 'textfield',
					'heading'     => esc_attr__( 'Full Image ID', 'fusion-builder' ),
					'description'  => esc_attr__( 'Upload an image that will show up in the lightbox.', 'fusion-builder' ),
					'param_name'  => 'full_image_id',
					'value'       => '',
					'hidden'      => true,
				],				
				[
