	...
	if ( ! class_exists( 'FusionForm_Textarea' ) ) {
		...
		class FusionForm_Textarea extends Fusion_Form_Component {
			...
			public function render_input_field( $content ) {
				...
				$element_html .= $min_lenght . ' ' . $max_length . ' rows="' . $this->args['rows'] . '" tabindex="' . $this->args['tab_index'] . '" id="' . $this->args['name'] . '" name="' . $this->args['name'] . '"' . $element_data['class'] . $element_data['required'] . $element_data['readonly'] . $element_data['disabled'] . $element_data['placeholder'] . $element_data['style'] . $element_data['holds_private_data'] . '>' . $content . '</textarea>';
		...
		fusion_builder_frontend_data(
			...
			[
				...
				'params'         => [
					...
					[
						'type'        => 'radio_button_set',
						'heading'     => esc_attr__( 'Readonly Field', 'fusion-builder' ),
						'description' => esc_attr__( 'Choose to set the field to read-only, which makes its content uneditable. Readonly fields will be submitted.', 'fusion-builder' ),
						'param_name'  => 'readonly',
						'default'     => 'no',
						'value'       => [
							'yes' => esc_attr__( 'Yes', 'fusion-builder' ),
