		...
		class FusionForm_Rating extends Fusion_Form_Component {
			...
			public function render_input_field( $content ) {
				...
				while ( $limit > 0 ) {
					...
					$options .= '<i class="' . fusion_font_awesome_name_handler( $this->args['icon'] ) . '"></i>';
					$options .= '</label>';
					$limit--;
				}

				$element_open_html  = '<fieldset class="fusion-form-rating-area fusion-form-rating-area-' . $this->counter . '">';
				$element_close_html = '</fieldset>';
				$element_html      .=  '<div class="fusion-form-rating-wrapper' . ( is_rtl() ? ' rtl' : '' ) . '">' . $options . '</div>';
				...
				if ( '' !== $element_data['label'] ) {
					$tag = 'above' === $this->params['form_meta']['label_position'] ? 'legend' : 'div';
