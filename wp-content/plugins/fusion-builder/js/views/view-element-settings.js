		...
		FusionPageBuilder.ElementSettingsView = window.wp.Backbone.View.extend( {
			...
			openAISettings: function( event ) {
				...
				if ( 'textfield' === inputType ) {
					...
					text = input.val();
				} else if ( 'tinymce' === inputType ) {
					const contentTextarea = $option.find( '.fusion-editor-field' );
					const textareaID = contentTextarea.attr( 'id' );

					const buttons = $option.find( '.wp-editor-tabs' );
					const htmlBtn = buttons.find( '#element_content-html' );

					let isTinyMCE = true;
					if ( htmlBtn && 'true' === htmlBtn.attr( 'aria-pressed' ) ) {
						isTinyMCE = false;
					}

					if ( isTinyMCE ) {
						text = window.tinyMCE.get( textareaID ).getContent();
					} else {
						text = contentTextarea.val();
					}
				} else if ( 'textarea' === inputType ) {
					const textarea = $option.find( 'textarea' );
					text = textarea.val();
				}
				...
				const aiUpdateParam = ( newText, modifiedParams = [] ) => {
					if ( 'textfield' === inputType ) {
						...
						const contentTextarea = $option.find( '.fusion-editor-field' );
						const textareaID = contentTextarea.attr( 'id' );
						// Set content to both tinyMCE and text.
						window.tinyMCE.get( textareaID ).setContent( newText );
						contentTextarea.val( newText );
					}  else if ( 'textarea' === inputType ) {
						const textarea = $option.find( 'textarea' );
						textarea.val( newText );
					} else {
						simulateUserInput($option.find( '#' + param ), newText);
					}

					if ( modifiedParams.length ) {
						for ( const modifiedParam of modifiedParams ) {
							console.log( modifiedParam );
							simulateUserInput($option.closest('.fusion-builder-module-settings').find( '#' + modifiedParam.id ), String(modifiedParam.value));
						}
					}
				};
