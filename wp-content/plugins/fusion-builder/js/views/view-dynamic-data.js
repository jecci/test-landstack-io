/* global FusionPageBuilderApp, fusionDynamicData, FusionPageBuilderEvents */
	...
	initEditDynamic: function( $targetEl, open ) {
		...
		if ( fields ) {
			_.each( fields, function( field, id ) {
				...
				// Make sure ACF Text fuield is always of type text for Post Cards custom field option.
				if ( 'undefined' !== dynamic.id && 'acf_text' === dynamic.id && 'custom_field_name' === param && 'field' === id ) {
					attributes.field.type = 'text';
				}
		...
		$targetEl.on( 'click', '.dynamic-title', function() {
			jQuery( this ).parent().find( '.dynamic-param-fields' ).slideToggle( 300, function() {
				FusionPageBuilderEvents.trigger( 'fusion-dynamic-data-toggle', param );
			} );
