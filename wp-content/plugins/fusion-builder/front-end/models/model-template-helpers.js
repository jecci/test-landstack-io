...
_.mixin( {
	...
	awbHTMLAttributes: function( values, attributes, context = 'wrapper' ) {
		if ( 'undefined' === typeof values.html_attributes || '' === values.html_attributes ) {
			return attributes;
		}

		const htmlAttributes = JSON.parse( FusionPageBuilderApp.base64Decode( values.html_attributes ) );
		if ( 'object' === typeof htmlAttributes ) {
			_.each( htmlAttributes, function( attribute ) {
				if ( '' !== attribute.value ) {
					if ( 
						( 'wrapper' === context && ( 'undefined' === typeof attribute.node_toggle || '' === attribute.node_toggle || 'wrapper' === attribute.node_toggle ) ) ||
						( 'main_node' === context && 'undefined' !== typeof attribute.node_toggle && 'main_node' === attribute.node_toggle )
					) {
						if ( attribute.name.includes( 'class' ) || attribute.name.includes( 'style' ) ) {
