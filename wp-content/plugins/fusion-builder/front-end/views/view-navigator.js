    ...
    FusionPageBuilder.NavigatorView = Backbone.View.extend( {
        ...
        handleSortStart: function( event, ui ) {
            ...
            function getItemType() {
                ...
                if ( ui.item.hasClass( 'awb-builder-nav__list-item--special-container' ) ) {
        ...
        handleRemoveBtnClick: function( e ) {
            ...
            } else if ( view.removeCheckoutForm && 'fusion_woo_checkout_form' === view.model.get( 'element_type' ) ) {
                view.removeCheckoutForm();
            } else if ( view.removeNextPage && 'fusion_builder_next_page' === view.model.get( 'element_type' ) ) {
                view.removeNextPage();
        ...
        handleCloneBtnClick: function( e ) {
            ...
            if ( view.cloneSpecialElement ) {
                view.cloneSpecialElement( undefined );
            } else if ( view.cloneContainer ) {
                view.cloneContainer();
        ...
        handleAddBtnClick: function( e ) {
            ...
            } else if ( 'fusion_builder_form_step' === type ) {
                view.$el.find( '.fusion-builder-container-add' ).trigger( 'click' );
        ...
        _getNearbyItemToDropAndTarget( $navItemSorted ) {
            ...
            if ( 'before' === dropTargetPosition ) {
                if ( nearbyItemView.$el.hasClass( 'fusion-special-item' ) ) {
                    $dropTarget = nearbyItemView.$el.children().children( '.fusion-droppable.target-before' );
                } else {
                    $dropTarget = nearbyItemView.$el.children( '.fusion-droppable.target-before' );
                }
            } else if ( 'after' === dropTargetPosition ) {
                if ( nearbyItemView.$el.hasClass( 'fusion-special-item' ) ) {
                    $dropTarget = nearbyItemView.$el.children().children( '.fusion-droppable.target-after' );
                } else {
        ...
        openOrCloseNestedRowsNeeded: function( view, onlyFromInside = false ) {
            var $nestedRows            = 'undefined' !== typeof view.$el ? view.$el.closest( '.fusion-nested-columns' ) : {},
                viewIsNestedRows       = 'undefined' !== typeof view.$el ? view.$el.is( '.fusion-nested-columns' ) : {},
