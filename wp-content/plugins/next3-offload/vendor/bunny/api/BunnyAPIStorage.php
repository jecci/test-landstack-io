...
class BunnyAPIStorage extends BunnyAPI
{
    ...
    public function zoneConnect(string $storage_name, string $access_key = '', string $region = ''): void
    {
        $this->storage_name = $storage_name;
        (empty($access_key)) ? $this->findStorageZoneAccessKey($storage_name) : $this->access_key = $access_key;
        
        $hostname = self::HOSTNAME;
        if( !empty($region) && $region != 'de'){
            $hostname = $region . '.' . self::HOSTNAME;
            
            $this->host_name = $hostname;
            $this->storage_api_url = str_replace('https://', 'https://'. $region . '.', self::STORAGE_API_URL);
            $this->video_api_url = str_replace('https://', 'https://'. $region . '.', self::VIDEO_STREAM_URL);
        }
        
        if( !function_exists('ftp_connect')){
            next3_core()->provider_ins->saveLogs( json_encode( "ftp_connect not found, enable ftp in your server." ) );
            die("ftp_connect not found, enable ftp in your server.");
            return;
        }
        // header key
        $this->headers = [
        ...
        $conn_id = @ftp_ssl_connect($hostname, 21, 90);
        $login = @ftp_login($conn_id, $storage_name, $this->access_key);
        @ftp_pasv($conn_id, true);
        
        try {
            if (!$conn_id || !$login) {
                next3_core()->provider_ins->saveLogs( json_encode( "Could not make FTP connection to " . $hostname ) );
                return;
            }
    ...
    {
        $array = json_decode(file_get_contents($this->storage_api_url . "/$this->storage_name/" . $dir . "/?AccessKey=" . $this->access_key), true);
        $size = $files = 0;
        foreach ($array as $value) {
            if ($value['IsDirectory'] === false) {
                $size += $value['Length'];
    ...
    {
        $ftp_url = "ftp://$this->storage_name:$this->access_key@" . $this->host_name . "/$this->storage_name/$get_file";
    ...
    {
        $array = json_decode(file_get_contents($this->storage_api_url . "/$this->storage_name" . $dir_dl_from . "/?AccessKey=" . $this->access_key), true);
        $files_downloaded = 0;
        foreach ($array as $value) {
            if ($value['IsDirectory'] === false) {
                $file_name = $value['ObjectName'];
    ...
    public function uploadFile(string $upload, string $upload_as, int $mode = FTP_BINARY): array
    {
        if (ftp_put($this->connection, $upload_as, $upload, $mode)) {
            ...
            // upload via api
            $url = $this->storage_api_url . "/$this->storage_name/$upload_as";

            $ch = curl_init($url);

            curl_setopt($ch, CURLOPT_CUSTOMREQUEST, "PUT");
            curl_setopt($ch, CURLOPT_POSTFIELDS, file_get_contents($upload));
    ...
    {
        $ftp_url = "ftp://$this->storage_name:$this->access_key@" . $this->host_name . "/$this->storage_name/$upload_as";
    ...
    {
        return json_decode(file_get_contents($this->storage_api_url . "/$this->storage_name/?AccessKey=" . $this->access_key), true);
    }

    public function listFiles(string $location = ''): array
    {
        $array = json_decode(file_get_contents($this->storage_api_url . "/$this->storage_name" . $location . "/?AccessKey=" . $this->access_key), true);
    ...
    public function getStorageObjects( $location = ''){
        return json_decode(file_get_contents($this->storage_api_url . $location . "/?AccessKey=" . $this->access_key));
    }
