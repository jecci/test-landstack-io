...
class ClientResolver
{
    ...
    public static function _apply_token($value, array &$args)
    {
        ...
        if ($value instanceof Token) {
            ...
            throw new IAE('Token must be an instance of ' . 'Aws\\Token\\TokenInterface, an associative ' . 'array that contains "token" and an optional "expires" ' . 'key-value pairs, a token provider function, or false.');
        }
    }
    ...
    {
        if ($value === \false) {
            $value = new Configuration(\false, \Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_HOST, \Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_PORT, \Aws\ClientSideMonitoring\ConfigurationProvider::DEFAULT_CLIENT_ID);
            $args['csm'] = $value;
        }
    ...
    public static function _default_endpoint_provider(array $args)
    {
        ...
        if (self::isValidService($serviceName) && self::isValidApiVersion($serviceName, $apiVersion)) {
            ...
            return new \Aws\EndpointV2\EndpointProviderV2($ruleset, EndpointDefinitionProvider::getPartitions());
        }
        $options = self::getEndpointProviderOptions($args);
